(function () {
  ...
  function readJsonResponse(r) {
    return r.text().then(function (text) {
      ...
      try {
        ...
        if (r.status === 403) {
          hint =
            "HTTP 403 is not produced by this Node app — it comes from nginx, ModSecurity, Cloudflare WAF, or your host. Whitelist POST to /save-profile and related routes, or add a WAF exception for JSON bodies. If your server log shows no line for this POST, the request never reached Node.";
        }
  ...
  function buildProfileFromForm() {
    ...
    return {
      ...
      tagline: (document.getElementById("tagline") && document.getElementById("tagline").value.trim()) || "",
